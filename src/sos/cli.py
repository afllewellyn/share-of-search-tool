...
def _render_config_yaml(
    ...
    def block(name: str, keywords: List[str], indent: str) -> str:
        lines = [f"{indent}name: {name}"]
        lines.append(f"{indent}keywords:")
        lines.extend(f"{indent}  - {k}" for k in keywords)
        lines.append(f"{indent}ambiguous: false")
        return "\n".join(lines)
    ...
    for competitor in competitors:
        lines.append(f"  - name: {competitor}")
        lines.append("    keywords:")
        lines.append(f"      - {competitor.lower()}")
...
def run(
    ...
    brand_frame, warnings = transform.build_brand_frame(rows, config)
    for warning in warnings:
        _warn(warning)

    new_rows = store_module.build_rows(brand_frame, config, data_source=source.name)
    combined = store_module.upsert(data_dir, new_rows, config.smoothing_windows)
