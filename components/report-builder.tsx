...
export default function ReportBuilder() {
  ...
  useEffect(() => {
    ...
    if (!iframe) return
    const resize = () => { const doc = iframe.contentDocument; if (!doc) return; iframe.style.height = `${Math.max(560, doc.documentElement.scrollHeight, doc.body?.scrollHeight ?? 0)}px` }
    iframe.addEventListener('load', resize)
    resize()
    const timers = [500, 1500, 3000].map((ms) => window.setTimeout(resize, ms))
    return () => { iframe.removeEventListener('load', resize); timers.forEach(clearTimeout) }
  }, [report])
