...
type Row = { brand: string; is_own_brand: boolean; sos_pct: number; raw_volume: number; mom_delta_pp: number; yoy_delta_pp: number; rank: number; rank_change: number; exceeds_noise_threshold?: boolean; data_gap?: boolean; ambiguous?: boolean }
type Report = { html: string; own_brand: string; market: string; latest: { month: string; month_label: string; rows: Row[] }; commentary: string[]; warnings: string[]; months_requested: number; months_returned: number; cost_usd: number }

const fallbackMarkets = ['US', 'UK', 'AU', 'CA']
const fallbackMonths = [12, 24, 48]
...
function delta(value: number) { return `${value > 0 ? '+' : ''}${value.toFixed(1)} pp` }

export default function ReportBuilder() {
  const [markets, setMarkets] = useState(fallbackMarkets)
  const [monthsOptions, setMonthsOptions] = useState(fallbackMonths)
  const [limits, setLimits] = useState({ max_competitors: 12, max_keywords_per_brand: 8 })
  ...
  useEffect(() => {
    fetch('/api/markets').then(async (response) => {
      const data = await readJson<MarketPayload>(response)
      setMarkets(data.markets?.length ? data.markets : fallbackMarkets)
      setMonthsOptions(data.months?.length ? data.months : fallbackMonths)
  ...
  useEffect(() => {
    ...
    if (!iframe) return
    const resize = () => { iframe.style.height = `${Math.max(560, iframe.contentDocument?.documentElement.scrollHeight ?? 560)}px` }
  ...
  if (report) return <Results report={report} iframeRef={iframeRef} onReset={reset} />

  return <main className="min-h-screen bg-ink px-5 py-6 text-slate-100 sm:px-8 lg:px-12">
