...
type Brand = { name: string; keywords: string; url?: string; ambiguous?: boolean }
type MarketPayload = { passphrase_required?: boolean; markets: string[]; months: number[]; limits: { max_competitors: number; max_keywords_per_brand: number } }
...
export default function ReportBuilder() {
  ...
  useEffect(() => { try { setPassphrase(sessionStorage.getItem('sos-passphrase') ?? '') } catch {} }, [])
  ...
  useEffect(() => {
    fetch('/api/markets')
      ...
      .then((loaded) => {
        setOptions(loaded)
        // Keep the controlled values inside what the API will accept.
        setMarket((current) => (loaded.markets.includes(current) ? current : loaded.markets[0] ?? current))
        setMonths((current) => (loaded.months.includes(current) ? current : loaded.months[0] ?? current))
      })
  ...
  async function submit(event: React.FormEvent) {
    ...
    if (options.passphrase_required && !passphrase.trim()) return setError('Enter the access passphrase.')
    try { sessionStorage.setItem('sos-passphrase', passphrase) } catch {}
    setLoading(true)
    try {
      const response = await fetch('/api/run', { method: 'POST', headers: { 'Content-Type': 'application/json', 'X-SOS-Passphrase': passphrase }, body: JSON.stringify({ own_brand: { name: ownBrand.name.trim(), keywords: splitKeywords(ownBrand.keywords), url: ownBrand.url?.trim() || undefined, ambiguous: ownBrand.ambiguous || undefined }, competitors: validCompetitors.map((item) => ({ name: item.name.trim(), keywords: splitKeywords(item.keywords), ambiguous: item.ambiguous || undefined })), market, months }) })
  ...
  return <main className="min-h-screen px-5 py-6 text-slate-100 sm:px-8 lg:px-12">
    <div className="mx-auto max-w-6xl">
      <header className="flex items-center justify-between border-b border-line pb-5"><div className="flex items-center gap-3"><div className="grid size-9 place-items-center rounded-xl bg-signal text-ink"><TrendingUp className="size-5" /></div><span className="font-mono text-sm font-bold tracking-[0.2em] text-signal">SOS / REPORT BUILDER</span></div><span className="hidden font-mono text-xs text-muted sm:block">A LEADING INDICATOR OF MARKET SHARE</span></header>
      <section className="grid gap-8 py-12 lg:grid-cols-[1.05fr_0.95fr] lg:items-end lg:py-20"><div><p className="mb-5 font-mono text-xs uppercase tracking-[0.2em] text-signal">Share of Search</p><h1 className="max-w-3xl text-balance text-5xl font-semibold leading-[0.98] tracking-[-0.06em] sm:text-7xl">Are more people searching for you, or for your competitors?</h1><p className="mt-6 max-w-xl text-pretty text-lg leading-8 text-muted">Share of Search measures interest in your brand relative to the brands you compete with: of everyone searching for any of you by name, how many are searching for you. Tracked monthly, it tends to move before market share does.</p></div><div className="border-l border-signal/40 pl-5 text-sm leading-6 text-muted"><p className="font-mono text-xs uppercase tracking-[0.18em] text-signal">Why it matters</p><p className="mt-3">In the IPA research behind this method, Les Binet found that share of search leads share of market by months. Think of it as brand consideration measured from real behaviour: no survey, no panel, just what people type.</p></div></section>
      <form onSubmit={submit} className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
        <section className="rounded-2xl border border-line bg-panel/80 p-6 sm:p-8"><div className="mb-8 flex items-start justify-between gap-4"><div><p className="font-mono text-xs tracking-[0.18em] text-signal">01 / OWN BRAND</p><h2 className="mt-2 text-2xl font-semibold tracking-tight">Start with the brand you own</h2></div><span className="rounded-full border border-line px-3 py-1 font-mono text-[10px] text-muted">REQUIRED</span></div><div className="grid gap-5"><label className="grid gap-2 text-sm text-muted">Brand name<input value={ownBrand.name} onChange={(e) => setOwnBrand({ ...ownBrand, name: e.target.value })} placeholder="e.g. Acme" className="field" /></label><label className="grid gap-2 text-sm text-muted">Search keywords <span className="text-xs text-muted">Comma-separated, up to {limits.max_keywords_per_brand}</span><input value={ownBrand.keywords} onChange={(e) => setOwnBrand({ ...ownBrand, keywords: e.target.value })} placeholder="acme, acme app, acme software" className="field" /></label><label className="grid gap-2 text-sm text-muted">Website <span className="text-xs text-muted">Optional context for the report</span><input type="url" value={ownBrand.url} onChange={(e) => setOwnBrand({ ...ownBrand, url: e.target.value })} placeholder="https://acme.com" className="field" /></label><label className="flex items-center gap-3 text-sm text-muted"><input type="checkbox" checked={!!ownBrand.ambiguous} onChange={(e) => setOwnBrand({ ...ownBrand, ambiguous: e.target.checked })} className="size-4 accent-signal" />Common word (e.g. Apple, Orange): flag searches that may not be for this brand</label></div></section>
        ...
        <section className="rounded-2xl border border-line bg-panel/80 p-6 sm:p-8 lg:col-span-2"><div className="flex items-start justify-between gap-4"><div><p className="font-mono text-xs tracking-[0.18em] text-signal">03 / COMPETITORS</p><h2 className="mt-2 text-2xl font-semibold tracking-tight">Name the brands you compete with</h2><p className="mt-2 text-sm leading-6 text-muted">Keep keyword coverage fair: include the variants people actually search for.</p></div><span className="font-mono text-xs text-muted">{competitors.length} / {limits.max_competitors}</span></div><div className="mt-6 grid gap-3">{competitors.map((competitor, index) => <div key={index} className="grid gap-3 rounded-xl border border-line/70 bg-ink/30 p-3 sm:grid-cols-[0.7fr_1fr_auto_auto]"><input id={`competitor-${index}-name`} aria-label={`Competitor ${index + 1} name`} value={competitor.name} onChange={(e) => updateCompetitor(index, { name: e.target.value })} placeholder={`Competitor ${index + 1}`} className="field" /><input id={`competitor-${index}-keywords`} aria-label={`Competitor ${index + 1} keywords`} value={competitor.keywords} onChange={(e) => updateCompetitor(index, { keywords: e.target.value })} placeholder="brand keywords, variants" className="field" /><label className="flex items-center gap-2 whitespace-nowrap text-xs text-muted" title="A common word: searches may not be for this brand"><input type="checkbox" checked={!!competitor.ambiguous} onChange={(e) => updateCompetitor(index, { ambiguous: e.target.checked })} className="size-4 accent-signal" />Common word</label><button type="button" aria-label={`Remove competitor ${index + 1}`} onClick={() => setCompetitors(competitors.filter((_, i) => i !== index))} disabled={competitors.length === 1} className="grid size-11 place-items-center rounded-lg border border-line text-muted transition hover:border-red-300 hover:text-red-200 disabled:cursor-not-allowed disabled:opacity-30"><Trash2 className="size-4" /></button></div>)}</div><button type="button" onClick={() => setCompetitors([...competitors, emptyCompetitor()])} disabled={competitors.length >= limits.max_competitors} className="mt-4 inline-flex items-center gap-2 rounded-lg border border-dashed border-line px-4 py-3 font-mono text-xs text-muted transition hover:border-signal hover:text-signal disabled:opacity-40"><Plus className="size-4" /> Add competitor</button></section>
        {options.passphrase_required && <section className="rounded-2xl border border-line bg-panel/80 p-6 sm:p-8 lg:col-span-2"><p className="font-mono text-xs tracking-[0.18em] text-signal">04 / ACCESS</p><h2 className="mt-2 text-2xl font-semibold tracking-tight">Enter the passphrase</h2><p className="mt-2 text-sm text-muted">Each run spends money on the owner&apos;s account, so reports are limited to people who have been given the passphrase.</p><label className="mt-6 grid gap-2 text-sm text-muted">Passphrase<input type="password" autoComplete="current-password" value={passphrase} onChange={(e) => setPassphrase(e.target.value)} className="field max-w-sm" /></label></section>}
        ...
        <div className="flex flex-col gap-4 lg:col-span-2 sm:flex-row sm:items-center sm:justify-between"><p className="text-xs leading-5 text-muted">Your data stays in this report flow. No account or database required.</p><button disabled={loading} className="group inline-flex min-h-14 items-center justify-center gap-3 rounded-xl bg-signal px-6 font-semibold text-ink transition hover:bg-signal/90 disabled:cursor-wait disabled:opacity-70">{loading ? <><LoaderCircle className="size-5 animate-spin" /> Pulling {months} months from Google, ~5–10 s</> : <>Run report<ArrowRight className="size-5 transition group-hover:translate-x-1" /></>}</button></div>
      </form>
      <footer className="mt-16 border-t border-line py-6 text-sm leading-6 text-muted"><p>Built by <a href="https://www.linkedin.com/in/afllewellyn" target="_blank" rel="noopener noreferrer" className="text-slate-200 underline decoration-line underline-offset-4 hover:text-signal">Andrew Llewellyn</a> · <a href="https://github.com/afllewellyn/share-of-search-tool" target="_blank" rel="noopener noreferrer" className="text-slate-200 underline decoration-line underline-offset-4 hover:text-signal">Source on GitHub</a> · <Link href="/about" className="text-slate-200 underline decoration-line underline-offset-4 hover:text-signal">About Share of Search</Link></p><p className="mt-3">Methodology follows Les Binet, IPA EffWorks 2020. Share of Search tends to lead market share; read it as an early signal, not a forecast. Every run spends one paid request.</p></footer>
    </div>
  </main>
}
...
function Results({ report, onReset }: { report: Report; onReset: () => void }) {
  ...
  const downloadHtml = useCallback(() => {
    ...
    link.href = url
    link.download = 'share-of-search.html'
    link.click()
    URL.revokeObjectURL(url)
  }, [report.html])

  // The dashboard's own download button cannot save from inside the sandbox,
  // so it asks this page to do it. Only messages from our iframe count.
  useEffect(() => {
    const onMessage = (event: MessageEvent) => {
      if (event.source !== iframeRef.current?.contentWindow) return
      if (event.data?.type === 'sos:download-html') downloadHtml()
    }
