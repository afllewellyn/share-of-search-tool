...
The CLI is the full tool. The web interface is a second front door for people who
won't open a terminal: a hosted form (brand, keywords, competitors, market, history)
that runs once and returns the same dashboard `sos dashboard` writes.

It is the same pipeline — `sos.web.run_request` is `refresh()` followed by
`build_dashboard()` over a throwaway store — with three differences:

- **Every submission is one paid API request** (~$0.075), using the credentials on the
  deployment, not the visitor's. There is no dry run and nothing is cached.
- **It is always a backfill.** No store persists between runs, so there are no refresh
...
- **Runs are rate limited**: 6 per address and 30 per function instance every ten
  minutes (429 after). Optionally set `SOS_WEB_PASSPHRASE` to close the form to people
  who have the passphrase; the page shows the field only when one is configured.
...
without them `/api/run` answers 500 with a clear message. `SOS_WEB_PASSPHRASE` is optional. Vercel installs the Python
...
Scheduled runs with commit-back · ESOS (needs market-share data) · paid
